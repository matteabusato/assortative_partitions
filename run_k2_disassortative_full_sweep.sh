...
#SBATCH --array=0-27%4
#SBATCH --output=job_output/k2_dis_full_%A_%a.out
#SBATCH --error=job_output/k2_dis_full_%A_%a.err
...
mkdir -p job_output
mkdir -p results/k2_disassortative_full_sweep
...
export PD_OUTPUT_DIR="${PD_OUTPUT_DIR:-${PROJECT_ROOT}/results/k2_disassortative_full_sweep}"
...
srun --cpu-bind=cores \
    python -u src/run_k2_disassortative_full_sweep.py
