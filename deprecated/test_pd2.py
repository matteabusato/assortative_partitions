import numpy as np
from assortative_partitions.deprecated.pd_3_assortative import population_dynamics_3ass

my_chi = np.array([
    [
      0.16432256810879467,
      0.08450537753499858,
      0.08450537785183071
    ],
    [
      0.08450538717654685,
      0.1643225755268645,
      0.0845053860714727
    ],
    [
      0.08450538190925244,
      0.08450538048734998,
      0.16432256533288964
    ]
  ])

my_chi = my_chi / my_chi.sum()

pd = population_dynamics_3ass(
    d=7, H=4,
    m_parisi=1.0,
    mu=np.zeros(3),
    M=10000,
    init_type='bp_fixed_point_noisy',
    init_chi_bp=my_chi,
    init_noise=1e-8,
    max_iter=2000, tol=0.05,
    sampling_threshold=1000, sampling_interval=50,
    seed=0,
    # --- wandb ---
    use_wandb=True,
    wandb_project='pop_dyn_3ass',
    wandb_group='d7_H4_stability',
    wandb_name='bp_fp_noisy_1e-8',
    wandb_config_extra={
        'chi_init': my_chi.tolist(),
        'test_type': 'RS_stability_from_BP_fixed_point',
    },
    log_every=50,
    log_histograms_every=200,
)
pd.run(verbose=2)
print(pd)
pd.save()
pd.wandb_finish()   # important: flush and close the wandb run