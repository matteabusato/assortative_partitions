...
class Factorial():
    def __init__(self, D, H):
        ...
        factorials = np.zeros((D-H+2, D-H+2, D-H+2, D-H+2))
            ...
            for i2 in range(D-H+2-r1):
                ...
                for i3 in range(D-H+2-r1-r2):
                    ...
                    for i4 in range(D-H+2-r1-r2-r3):
                        ...
                        factorials[i1][i2][i3][i4] = c1 * c2 * c3 * c4

        self.factorials = factorials                  

    def get_factorial_chi(self, r1, r2, r3, r4):
        return self.factorials[r1-self.H+1][r2][r3][r4] * (self.D-r1-r2-r3-r4) / self.D
    
    def get_factorial_Z_node(self, r1, r2, r3, r4):
        return self.factorials[r1-self.H+1][r2][r3][r4]
...
def initialize_chi(initialization_chi='gaussian', epsilon=0.5):
        ...
        if initialization_chi == "uniform":
            chi = np.ones((5,5), dtype=float)
        elif initialization_chi == "gaussian":
            chi = np.random.rand(5, 5)
        elif initialization_chi == "almost_unif":
            chi = np.random.randn(5,5) / 100 + np.ones((5,5))

        return normalize_chi(chi)

def find_current_mu(D, M_star, chi, mu0=np.zeros(5), loss='linear', settingmu='previous'):
    if settingmu == "zero":
        mu0 = np.zeros(5, dtype=float)
...
def update_chi(D: int, H: int, M, THRESHOLD, MAX_ITER, chi, damping, mu0, settingmu, loss, FACTORIALS):
    ...
    if settingmu != "always_zero":
        ...
        mu = np.array([0, 0, 0, 0, 0], dtype=float)
            ...
            for r2 in range(0, D - r1):
                for r3 in range(0, D-r1-r2):
                    for r4 in range(0, D-r1-r2-r3):
                        term = (chi[f[0], i] ** r1) * (chi[f[1], i] ** r2) * (chi[f[2], i] ** r3) * (chi[f[3], i] ** r4) * (chi[f[4], i] ** (D - 1 - r1 - r2 - r3 - r4))
                        second_term += FACTORIALS.get_factorial_chi(r1, r2, r3, r4) * term
            ...
            for r2 in range(0, D - r1 + 1):
                for r3 in range(0, D - r1 - r2 + 1):
                    for r4 in range(0, D - r1 - r2 - r3 + 1):
                        term = (chi[f[0], i] ** r1) * (chi[f[1], i] ** r2) * (chi[f[2], i] ** r3) * (chi[f[3], i] ** r4) * (chi[f[4], i] ** (D - r1 - r2 - r3 - r4))
                        Z_node += FACTORIALS.get_factorial_Z_node(r1, r2, r3, r4) * term
    return Z_node
...
def compute_m_actual(D: int, mu, Z_edge, chi):
    m_actual = np.array([0, 0, 0, 0, 0], dtype=float)
...
if __name__ == "__main__":
    ...
    if experiment == "TEST1":
        Ds = [7]
        Hs = [[1, 2, 4, 5]]
        Ms = [np.array([0.5, 0.5, 0, 0, 0], dtype=float)]
        ...
        Ds = [7]
        Hs = [[1, 2, 3, 4, 5, 6]]
        Ms = [np.array([1/5, 1/5, 1/5, 1/5, 1/5], dtype=float)]
                ...
                for M in Ms:
                    ...
                    if USE_WANDB:
                        wandb.init(
                            ...
                            name=f"D{D}_H{H}_{initialization_chi}_{id_run}_F", # _F for fixed (index problem in update chi)
                            # group=f"D{D}_{experiment}_{settingmu}", 
                            group=f"5ASS_{experiment}", 
