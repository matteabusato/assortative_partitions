...
def main() -> None:
    ...
    output_root = Path(
        os.environ.get(
            "PD_OUTPUT_DIR",
            str(PROJECT_ROOT / "results" / "k4_assortative_full_sweep"),
        )
    )

    run_name = (
        f"K4_assortative_d{d}_H{H}_{expected_phase}"
    ...
    pd = PopDyn(
        ...
        wandb_group="K4_assortative_full_sweep",
        wandb_name=run_name,
        wandb_config_extra={
            "config_index": config_index,
            "expected_phase": expected_phase,
            "sweep_name": "K4_assortative_full_sweep",
        },
        log_every=200,
