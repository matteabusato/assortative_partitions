...
class PopDyn:
    ...
    def _same_count_coefficients(self, parent_messages: np.ndarray) -> np.ndarray:
        B = parent_messages.shape[0]
        P = parent_messages.shape[1]

        if P == 0:
            return np.ones((B, self.K, 1), dtype=self.dtype)

        idx = np.arange(self.K)

        same = parent_messages[:, :, idx, idx]
        col_sum = parent_messages.sum(axis=2)
        different = col_sum - same

        coeff = np.zeros((B, self.K, P + 1), dtype=self.dtype)
        coeff[:, :, 0] = 1.0
        ...
        for l in range(P):
            ...
            new_coeff[:, :, :l + 1] += coeff[:, :, :l + 1] * diff_l[:, :, None]
            new_coeff[:, :, 1:l + 2] += coeff[:, :, :l + 1] * same_l[:, :, None]
    ...
    def _candidate_messages(self, parent_messages: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        B = parent_messages.shape[0]
        P = self.num_parents

        coeff = self._same_count_coefficients(parent_messages)
        r_values = np.arange(P + 1)

        mask_diag = self._constraint_mask_from_same_count(r_values + 1)
        mask_offdiag = self._constraint_mask_from_same_count(r_values)

        diag_values = np.sum(coeff * mask_diag[None, None, :], axis=2)
        offdiag_values = np.sum(coeff * mask_offdiag[None, None, :], axis=2)

        candidates = np.repeat(offdiag_values[:, :, None], self.K, axis=2)

        idx = np.arange(self.K)
        candidates[:, idx, idx] = diag_values

        if np.any(self.mu != 0.0):
            field = np.exp(-(self.mu[:, None] + self.mu[None, :]) / self.d)
            candidates *= field[None, :, :]

        Z = candidates.sum(axis=(1, 2))
        return candidates, Z


    def _randomly_permute_colors(self, messages: np.ndarray) -> np.ndarray:
        B = messages.shape[0]

        perms = np.array(
            [self.rng.permutation(self.K) for _ in range(B)],
            dtype=int,
        )
        ...
        return messages[batch_idx, row_idx, col_idx]
    ...
    def step(self) -> None:
        ...
        if self.num_parents == 0:
            ...
            parent_indices = self.rng.integers(
                low=0,
                high=self.M,
                size=self.num_parent_draws,
            )

            parent_messages = self.population[parent_indices].reshape(
                self.num_updated,
                self.num_parents,
                self.K,
                self.K,
        ...
        candidates, Z = self._candidate_messages(parent_messages)
        ...
        weights = np.power(Z, self.mparisi)
        weight_sum = weights.sum()

        if not np.isfinite(weight_sum) or weight_sum <= self.eps:
            self.no_update = True
            self.iteration += 1
            return
        ...
        selected = self.rng.choice(
            len(candidates),
            size=self.num_updated,
            replace=True,
            p=weights,
        )

        new_messages = candidates[selected] / Z[selected, None, None]

        if self.impose_color_symmetry:
            new_messages = self._randomly_permute_colors(new_messages)
    ...
    def diff(self, n_bins: Optional[int] = None) -> float:
        ...
        for a in range(self.K):
            for b in range(self.K):
                ...
                old_hist = old_hist.astype(self.dtype) / self.M
                new_hist = new_hist.astype(self.dtype) / self.M

                total += np.sum(np.abs(new_hist - old_hist))

        return float(total)


    def _node_partition_terms(
        self,
        node_messages: np.ndarray,
    ) -> Tuple[np.ndarray, np.ndarray]:
        coeff = self._same_count_coefficients(node_messages)
        P = node_messages.shape[1]

        r_values = np.arange(P + 1)
        mask = self._constraint_mask_from_same_count(r_values)

        color_weights = np.sum(coeff * mask[None, None, :], axis=2)

        if np.any(self.mu != 0.0):
            color_weights *= np.exp(-self.mu)[None, :]

        Z_node = color_weights.sum(axis=1)

        return color_weights, Z_node
    ...
    def update_observables(self, num_samples: Optional[int] = None) -> None:
        ...
        node_indices = self.rng.integers(
            low=0,
            high=self.M,
            size=node_num_samples * self.d,
        )

        node_messages = self.population[node_indices].reshape(
            node_num_samples,
            self.d,
            self.K,
            self.K,
        ...
        edge_indices = self.rng.integers(
            low=0,
            high=self.M,
            size=edge_num_samples * 2,
        )

        edge_messages = self.population[edge_indices].reshape(
            edge_num_samples,
            2,
            self.K,
            self.K,
        ...
        msg_1 = edge_messages[:, 0]
        msg_2 = edge_messages[:, 1]

        Z_edge = np.sum(msg_1 * np.swapaxes(msg_2, 1, 2), axis=(1, 2))

        valid_node = Z_node > self.eps
        valid_edge = Z_edge > self.eps

        node_power = np.zeros_like(Z_node, dtype=self.dtype)
        edge_power = np.zeros_like(Z_edge, dtype=self.dtype)

        node_power[valid_node] = np.power(Z_node[valid_node], self.mparisi)
        edge_power[valid_edge] = np.power(Z_edge[valid_edge], self.mparisi)

        Z_node_mean = np.mean(node_power)
        Z_edge_mean = np.mean(edge_power)

        if Z_node_mean <= self.eps or Z_edge_mean <= self.eps:
            self.psi = -np.inf
            self.phi = np.nan
        ...
        log_Z_node = np.zeros_like(Z_node, dtype=self.dtype)
        log_Z_edge = np.zeros_like(Z_edge, dtype=self.dtype)

        log_Z_node[valid_node] = np.log(Z_node[valid_node])
        log_Z_edge[valid_edge] = np.log(Z_edge[valid_edge])

        Z_node_log_mean = np.mean(node_power * log_Z_node)
        Z_edge_log_mean = np.mean(edge_power * log_Z_edge)

        self.psi = float(
            np.log(Z_node_mean) - 0.5 * self.d * np.log(Z_edge_mean)
        )

        self.phi = float(
            Z_node_log_mean / Z_node_mean
            - 0.5 * self.d * Z_edge_log_mean / Z_edge_mean
        )

        self.complexity = float(self.psi - self.mparisi * self.phi)

        rho_weights = np.zeros_like(Z_node, dtype=self.dtype)

        if self.mparisi == 0.0:
            rho_weights[valid_node] = 1.0 / Z_node[valid_node]
        else:
            rho_weights[valid_node] = np.power(
                Z_node[valid_node],
                self.mparisi - 1.0,
            )

        rho_num = np.mean(color_weights * rho_weights[:, None], axis=0)
        self.rho = rho_num / Z_node_mean

        self.s = float(self.phi)    
