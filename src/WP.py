...
class KStateSurveyPropagation:
    ...
    def __init__(
        ...
        # DP state encoding. Must hold counts up to d (used in
        # node_compatibility_probability with n_inputs = d), not just n_in.
        self.count_base = self.d + 1
...
def _save_run_json(
    ...
    where tag = "{ass|dis}_K{K}_d{d}_H{H}". A relative save_dir is resolved
    against the project root (parent of src/), not the current working
    directory, so the path is stable regardless of where the script is run.
    """
...
if __name__ == "__main__":
    # ---------------------------------------------------------------
    # Benchmark against the paper.
    # For K=2, d=8, H_ass=5, this should give:
    #
    #     Phi_SP ≈ 0.02302067747
    #
    # matching the table value 0.02302.
    # ---------------------------------------------------------------
    # benchmark_paper_values()

    # ---------------------------------------------------------------
    # Coloring sanity check:
    # K=3, H=1, disassortative = 3-coloring.
    # The don't-care matrix should be off-diagonal.
    # ---------------------------------------------------------------
    K = 3
    d = 8
    H = 5
    ...
    print("\nModel-dependent don't-care warning for K=3, H=1 coloring:")
    sp.print_dont_care_warning()
    ...
    eta_star = result.eta.to(sp.device, dtype=sp.dtype)
    phi_sp, Z_node, Z_edge = sp.sp_complexity(eta_star)

    print("\nFinal K=3, d=5, H=1 disassortative/coloring:")
    print("eta_dont_care:", sp.dont_care_probability(eta_star))
    print("eta_all_one:  ", sp.all_one_probability(eta_star))
    print("eta_zero:     ", sp.zero_probability(eta_star))
    print("Phi_SP:       ", phi_sp)
    print("Z_node:       ", Z_node)
    print("Z_edge:       ", Z_edge)
    print("residual:     ", result.history[-1]["residual_l1_half"])

    sp.print_nonzero_warnings(eta_star, threshold=1e-8)

    # ---------------------------------------------------------------
    # Save a single run to disk. Output path:
    #   results/wp/{ass|dis}_K{K}_d{d}_H{H}/<tag>_{eps}_{timestamp}.json
    # ---------------------------------------------------------------
    run_single(
        K=K,
        d=d,
    ...
    # ---------------------------------------------------------------
    # Optional scan over K=2 and K=3 with saving.
    # ---------------------------------------------------------------
    # rows = scan_K_H(
    #     Ks=(2, 3),
    #     d_max=14,
