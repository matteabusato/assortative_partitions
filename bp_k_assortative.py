...
def run_bp(K, D, H, M, THRESHOLD, MAX_ITER, chi, damping, log_every=1000, use_wandb=False):
    iter = 0
    t0 = time.time()
    ...
    while iter < MAX_ITER:
        chi_old = chi.copy()
        chi_new, mu = update_chi(K, D, H, M, THRESHOLD, MAX_ITER, chi, damping)
        ...
        if iter%log_every == 0:
            ...
            Z_node_tmp, Z_edge_tmp, phi_RS_tmp, m_actual_tmp, s_tmp = compute_quantities(K, D, H, chi, mu)
    ...
    for _ in range(N_RUNS):
        ...
        if USE_WANDB:
            wandb.init(
                ...
                name=f"N{N}_D{D}_H{H}_K{K}_damp{DAMPING}_seed{SEED}",
                group=f"N{N}_D{D}_H{H}_K{K}_M{M}", 
                config={
                    "K": K,
                    "N": N,
                    "D": D,
        ...
        chi, mu, iters, total_time, converged = run_bp(K, D, H, M, THRESHOLD, MAX_ITER, chi, DAMPING, log_every=LOG_EVERY,
            use_wandb=USE_WANDB)

        Z_node, Z_edge, phi_RS, m_actual, s = compute_quantities(K, D, H, chi, mu)
